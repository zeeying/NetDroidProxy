...
//			HttpMethod requestGet=new GetMethod(url);
//
//			requestGet.addRequestHeader("X-Android-MODEL", Build.MODEL);
//			requestGet.addRequestHeader("X-Android-SDK_INT", Integer.toString(Build.VERSION.SDK_INT));
//			requestGet.addRequestHeader("X-Android-RELEASE", Build.VERSION.RELEASE);
//			requestGet.addRequestHeader("X-App-Version", AppVersion);
//			requestGet.addRequestHeader("X-App-Install-ID", AppInstallID);
//			requestGet.addRequestHeader("User-Agent", System.getProperty("http.agent"));
//			Res response=client.execute(requestGet);
//
...
//			String[] lines=configString.split("\\n");
	...
	public void loadFromUrl(String url) throws Exception {
		...
		for (String line : lines) {
			...
			try {
				...
				throw new Exception(String.format("SmartProxy config file parse error: line:%d, tag:%s, error:%s",
						lineNumber, tagString, e));
			}
	...
	private boolean convertToBool(String valueString) {
		...
		if (valueString.equals("on") || valueString.equals("1") || valueString.equals("true") || valueString.equals
				("yes")) {
			return true;
		} else {
			return false;
